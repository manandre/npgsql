...
(
  ...
  CONSTRAINT ""pk_OrganisatieQmo_Organisatie_QueryModelObjects_Imp"" PRIMARY KEY (""Id"")
        ...
        class Invoice
        {
            ...
            public NpgsqlDate PurchaseDate { get; set; }
        ...
        class InvoiceItem
        {
            ...
            public NpgsqlDate? ReturnDate { get; set; }
            public decimal UnitPrice { get; set; }
            public int? ShipmentId { get; set; }
        }
        
        //const string ConnectionString = @"Server=127.0.0.1;Port=5432;Database=testdb;User Id=postgres;Password=0123456789;";
        void InitDatabase()
        {
            using var conn = OpenConnection();
...
DROP TABLE IF EXISTS InvoiceItem;
        ...
        void InsertInvoices(Invoice[] invoices)
        {
            ...
            using var writer = conn.BeginBinaryImport("COPY invoice(id, clientId, code, discount, purchaseDate, \"value\", paid, direction, creatorId) FROM STDIN (FORMAT BINARY)");
            foreach (var invoice in invoices)
            {
                var cols = new object[] { invoice.Id, invoice.ClientId, invoice.Code, invoice.Discount, invoice.PurchaseDate, invoice.Value, invoice.Paid, invoice.Direction, invoice.CreatorId };
        ...
        void HardCodedInsertInvoices(Invoice[] invoices)
        {
            using var conn = OpenConnection();
            using var writer = conn.BeginBinaryImport("COPY invoice(id, clientId, code, discount, purchaseDate, \"value\", paid, direction, creatorId) FROM STDIN (FORMAT BINARY)");
            foreach (var invoice in invoices)
            {
                writer.StartRow();
                writer.Write(invoice.Id, NpgsqlDbType.Integer);
                writer.Write(invoice.ClientId, NpgsqlDbType.Smallint);
                writer.Write(invoice.Code, NpgsqlDbType.Text);
                writer.Write(invoice.Discount, NpgsqlDbType.Real);
                writer.Write(invoice.PurchaseDate, NpgsqlDbType.Date);
                writer.Write(invoice.Value, NpgsqlDbType.Numeric);
                writer.Write(invoice.Paid, NpgsqlDbType.Numeric);
                writer.Write(invoice.Direction, NpgsqlDbType.Smallint);
                writer.Write(invoice.CreatorId, NpgsqlDbType.Text);
            }
            writer.Complete();
            writer.Close();
        }

        void InsertItems(InvoiceItem[] items)
        {
            using var conn = OpenConnection();
            using var writer = conn.BeginBinaryImport("COPY invoiceitem(id, invoiceId, commodityId, shipmentId, \"count\", unitPrice, returnDate) FROM STDIN (FORMAT BINARY)");
            foreach (var item in items)
            {
                var cols = new object[] { item.Id, item.InvoiceId, item.CommodityId, item.ShipmentId, item.Count, item.UnitPrice, item.ReturnDate };
                writer.WriteRow(cols);
            }
            writer.Complete();
            writer.Close();
        ...
        [Test, IssueLink("https://github.com/npgsql/npgsql/issues/2623")]
        public void Bug2623()
        
        {
            ...
            new Invoice
            {
                ...
                Discount = 0.1f,
                Paid = 10m,
                PurchaseDate = new NpgsqlDate(2049,4,4),
                Value = 100m
            },
            new Invoice
            {
                ...
                Discount = 0.2f,
                Paid = 20m,
                PurchaseDate = new NpgsqlDate(2050,5,5),
                Value = 200m
            }
            };
            ...
            new InvoiceItem
            {
                ...
                Count = 20,
                InvoiceId = 1,
                ReturnDate = new NpgsqlDate(2050, 5, 5),
                ShipmentId = 1,
                UnitPrice = 20m
            }
            };
            ...
            try
            {
                ...
                Console.WriteLine("Inserted invoices successfully.");
            }
            catch (Exception ex)
            {
                Console.WriteLine("Couldn't insert invoices successfully.");
            ...
            try
            {
                HardCodedInsertInvoices(invoices);
                Console.WriteLine("Inserted invoices successfully.");
            }
            catch (Exception ex)
            {
            ...
            {
                InsertItems(items);
                Console.WriteLine("Inserted items successfully.");
            }
            catch (Exception ex)
            {
                Console.WriteLine("Couldn't insert items successfully.");
                Console.WriteLine($"Error : {ex.Message}");
            }
